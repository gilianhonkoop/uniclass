...
import SearchOrderTable from "@/components/tables/SearchOrderTable";

export default function SearchOrder({ zoekOrder }: { zoekOrder: any }) {
  ...
  return (
    <>
        ...
        <form className="flex flex-col gap-3 items-left mt-1">
          ...
          <button
            ...
            formAction={async (form) => {
              const result = await zoekOrder(form);
              setResults(result);
            }}
          >
            zoeken
      ...
      <div className="mx-10 mt-5">
        ...
        {results.length > 0 && <SearchOrderTable orders={results} />}
      </div>
    </>
  );
}
